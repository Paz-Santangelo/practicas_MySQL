...
# Saldo maximo de los usuarios femeninos
SELECT MAX(saldo) 
FROM Usuarios 
WHERE sexo = 'M';
...
FROM clientes
JOIN provincias
ON clientes.codigo = provincias.codigo;

SELECT clientes.nombre AS 'CLIENTES', provincias.nombre AS 'PROVINCIAS' 
FROM clientes
JOIN provincias
ON clientes.codigo = provincias.codigo;

SELECT provincia.codigo,provincia.nombre FROM provincias AS provincia
LEFT JOIN clientes AS cliente
ON cliente.codigoProvincia = provincia.codigo
WHERE cliente.codigoprovincia IS null;
